 ...
 *
 * Notes
 * Need to find highest number in array first
 * Filter array to exclude any entries where the candidate's votes plus uncounted voters is not greater than the highest number
 * If there are two or more candidates with the highest number of votes, return 0
 */

function electionsWinners(votes: number[], k: number): number {
  const sortedArr = votes.sort((a: number, b: number) => b - a);
  const highestCount = sortedArr[0];
  const filteredArr = sortedArr.filter((votes) => {
    if (votes !== highestCount) {
      return votes + k > highestCount;
    }
    return true;
  });
  if (k === 0 && filteredArr[0] === filteredArr[1]) {
    return 0;
  }
